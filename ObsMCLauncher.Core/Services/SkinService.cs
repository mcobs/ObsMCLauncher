...
namespace ObsMCLauncher.Core.Services
{
    public class SkinService
    {
        ...
        private SkinService()
        {
            ...
            // 只记录路径，目录在真正要写入皮肤时才创建 ——
            // 纯离线账号永远不下皮肤，不该在数据目录里留下空的 cache/skins
        ...
        {
            // 离线账号没有任何皮肤来源（无 Minecraft UUID / 认证服务器），头像由
            // AccountAvatarService 本地生成（纯色底 + 用户名首字母）：
            // 必须在读皮肤缓存之前返回，否则历史遗留的 CachedSkinPath 会顶掉生成的头像。
            if (account.Type == AccountType.Offline) return null;
