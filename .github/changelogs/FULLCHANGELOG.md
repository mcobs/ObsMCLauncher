...
- 【插件API】新增 UiReady 事件与 IsUiReady 状态：主窗口打开、首页进入视觉树后广播一次，插件此时才能通过 GetUiRoot 修改界面
- 【插件】现在可以显示插件图标了，同时支持在插件市场中显示markdown文档
- 【插件】插件支持在线更新：列表显示「可更新」角标，可单个更新或一键全部更新，重启启动器后自动替换
- 【插件】更新带备份与失败自动回滚：替换失败会还原到旧版本并给出提示，插件不会因更新失败而消失
- 【插件】卸载时新增询问：可选择"连插件配置与数据一起删除"或"只移除插件本体（保留配置）"

### 修复
- 【版本】修复「从列表中移除」游戏目录对当前正在使用的目录无效的问题：移除后那一项会留在列表里（当前目录却已经切走）
- 【版本】游戏目录的「删除」改为「从列表中移除」：只把目录移出列表，不再删除磁盘上的实际文件夹，避免误删存档与模组
- 【设置】移除 JVM 参数内容框的默认预设文字，避免与预设方案混淆
- 【插件】插件数据目录从 `OMCL/plugins/{插件ID}` 改为 `OMCL/config/plugins/{插件ID}`：此前两者重合，导致更新插件会连带删掉插件配置（旧位置的历史文件不会自动迁移，需手动搬）
...
### 其他
- 【文档】更新插件开发指南，为每个 API 标注了引入版本，并补充「界面就绪时机」章节
...
### 新增
- 【背景】支持动图背景（GIF / 动画 WebP），可设置播放开关、帧率上限与解码长边上限
- 【背景】壁纸支持多图轮播，可自定义切换节奏、切换过渡、切换顺序，并支持拖动排序
- 【背景】支持将壁纸延伸到左侧导航栏，并单独调节导航栏透明度
- 【窗口】新增启动器自定义标题栏（图标/标题/最小化/最大化/关闭，支持拖动与双击最大化）
- 【设置/通用】设置页「窗口」页挂在「通用」二级导航下，新增「使用系统标题栏」开关
- 【向导】新增了五个页面：外观、通用、游戏、下载、完成

### 修复
- 【资源/更多】修复「滚动到底自动加载更多」始终未生效的问题
- 【资源】修复多处滚动相关的布局循环与加载失效问题
...
### 优化
- 【背景】失焦、最小化、电池供电与远程桌面下完全停止动图渲染，避免常驻唤醒 CPU
- 【背景】壁纸解码尺寸随窗口大小与缩放比例自适应重算，降低大图的内存占用
