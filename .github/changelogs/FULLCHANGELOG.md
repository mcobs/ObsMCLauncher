...
### 新增
- 新增首次启动欢迎窗口：首次启动先显示欢迎窗口（流程不过完不显示主界面），开场动画参考 ClassIsland 的 OOBE 实现（方块错峰上滑入场、3D 翻走的同时字母翻入、其中一个方块拉宽变蓝作品牌标记、字母间距收拢、整体缩放回落），随后欢迎内容淡入；未完成流程即关闭欢迎窗口视为取消启动
- 开发者控制台新增 welcome（手动打开欢迎窗口）与 welcome reset（重置首启标记）命令
- 欢迎流程未完成就手动关闭窗口时，弹出 FluentAvalonia 确认框提示"退出 ObsMCLauncher"，确认后才退出应用（崩溃流程不拦截）
- 欢迎页开场动画结束后，页面底部显示版本号与"数据迁移"链接（居中对齐、主题绿下划线，仿 ClassIsland；翻页后随页面隐藏）
- 欢迎流程新增开源许可页（仿 ClassIsland）：居中"同意许可条款"，勾选框内含蓝色下划线链接《ObsMCLauncher 开源许可》，点击弹出 Fluent 信息框展示许可全文，勾选后圆形下一步可用；正常"下一步"与数据迁移入口都先经过许可页
- 欢迎流程新增数据迁移页（仿 ClassIsland 数据迁移向导，仅导入）：Carousel 分步（选择来源 Plain Craft Launcher 2（2.13.0.0+） → 选择数据（游戏设置/应用设置） → 导入中 → 导入完成），迁移逻辑暂为占位

### 新增（实例级启动配置）
- init.json 新增实例级 JVM 参数与 Java 路径字段：版本设置页支持为单个版本自定义 Java 路径与 JVM 参数（跟随全局/自定义开关），启动时实例配置优先于全局配置
...
- 崩溃窗口、游戏日志窗口、开发者控制台窗口改用系统原生标题栏：移除各窗口自绘标题行与自绘关闭/最小化按钮，使用系统原生窗口按钮与拖拽，游戏日志动态标题改写到系统标题；关闭真实崩溃窗口即退出应用（开发者控制台 crash 指令的预览窗口不受影响）
- 欢迎窗口改用系统原生标题栏：移除自绘标题行（logo + 标题 + 关闭按钮）与拖拽/关闭 handlers，使用系统原生窗口按钮与拖拽；未完成流程关闭时仍走 OnClosing 确认框，确认退出才真正关窗并退出应用
- 全局通知提示框迁移到 FluentAvalonia InfoBar：通知类型映射为 InfoBar 严重级别（Info→Informational、Success/Warning/Error 对应同名级别、Progress/Countdown→Informational+进度条），severity 语义色、图标、关闭按钮由 InfoBar 提供并随深浅主题自动适配，移除 ViewModel 中硬编码的 #c58787 等类型色值（每处绑定都会新建画刷的 GC 抖动也随之消除）；进度/倒计时进度条以 3px 高度贴合卡片底部
- 通知动画架构重构：删除 NotificationItemViewModel 内约 150 行手写 Timer 帧动画（弹跳进场、滑动进场/退场、进度 lerp、滑动回弹），改为 NotificationCardView 的 XAML Transitions（透明度+位移动画）驱动进场/退场，进度条数值变化由 DoubleTransition 平滑补间；右下角模式保留右滑关闭（拖拽期间临时禁用过渡实现跟手，松手未达阈值由过渡动画回弹）
- 通知服务可靠性优化：自动关闭 Timer 集中管理并在移除/释放时统一停止（原实现不保存引用、手动关闭后仍会触发）；倒计时从 100ms 轮询累加改为 Stopwatch 计算剩余时间消除漂移；悬停通知时暂停自动关闭倒计时；居中横幅按类型设置默认时长（错误 5 秒/警告 4 秒/其余 3 秒，右下角仍尊重用户配置）；通知位置切换时同步更新已显示通知的交互逻辑；进度/倒计时通知关闭仍会取消关联任务
- Info 级通知背景修复：Fluent 默认的 Informational 背景为 50% 透明度（#80FFFFFF/#80F6F6F6），通知悬浮在内容上时难以阅读，改为不透明表面色（深色 #1C1F26 / 浅色 #FFFFFF，随主题切换）
- 设置页通知选项优化：手绘假窗口预览改为真实 InfoBar 样式预览（所见即所得），自动关闭时间输入框从 TextBox 改为 FluentAvalonia NumberBox（限制 3-30 秒），通知位置选中高亮色从硬编码蓝色改为启动器强调色
- 开发者控制台新增 notify 命令（notify [info/success/warning/error/progress/countdown]），用于手动验证通知卡片样式与动画

### 优化（版本实例页 / JVM 参数 / Java 选项）
- 版本实例页 Tab 选择栏接入应用主题色体系：tab 条背景与顶部标题栏统一（SurfaceBrush），选中 tab 与内容区共用页面底色合为一体，选中态文字主题绿加粗、悬停/按下状态改用冷色系表面色，消除 FluentAvalonia 默认暖灰（#282828 等）与冷色系主题的隔阂；深浅色模式同步适配（Theme.axaml 深色默认 + 主题切换时动态更新）
- 版本实例「版本设置」Java 选项升级：自定义模式下提供与设置页完全一致的 Java 探测列表（PATH/JAVA_HOME/常见安装目录，显示版本、位数、发行商，探测结果带 5 分钟缓存两页共用），新增「自定义路径...」选项、浏览按钮（浏览后自动识别版本信息，命中已探测条目则直接选中）与路径有效性校验提示；跟随全局时副标题显示全局实际生效的 Java 路径
- JVM 参数编辑器全面升级（版本实例页与设置页全局参数共用同一控件）：新增常用参数快捷开关（GC 选择、预分配内存、禁用显式 GC、字符串去重、优化字符串拼接、老版本 Forge 兼容等 10 项，点击即增删对应参数）、预设方案下拉（空白/均衡 G1GC/低内存优化/高配流畅/老版本兼容）、多行自由编辑与清空按钮；参数文本、开关状态、预设选中三方双向同步
- Java 探测逻辑从设置页抽离为共享服务 JavaOptionsProvider，设置页与版本实例页共用，保证两处 Java 列表一致并避免重复扫描

### 修复
- 修复版本实例页/账号管理页打开时内联弹窗闪现的问题：FluentAvalonia ContentDialog 主题用 :hidden 伪类的淡出动画隐藏弹窗，视图变为可见时该动画会先以不透明状态渲染一帧，导致分组管理/外置登录弹窗闪一下；现用始终隐藏的 Panel 包住弹窗，弹窗只在 ShowAsync 时移入遮罩层显示
