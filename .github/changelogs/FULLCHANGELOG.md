...
### 调整（主页自定义回退操作区组件化）
- 操作区去组件化：账号选择、版本选择、启动按钮、日志开关不再作为主页组件，改为主页/预览共用的固定操作区控件，"启动时显示游戏日志"开关回到启动按钮正下方；旧布局中残留的操作区组件读取时自动清理并收掉空行
- 移除插件自定义主页组件 API：`IPluginContext` 不再提供 `RegisterHomeComponent` / `UnregisterHomeComponent`，插件文档同步删除该章节，插件仍可通过 `RegisterHomeCard` 注册卡片
- 行容量限制：新增单行宽度上限常量，添加/移动组件前按行内组件最小宽度之和校验，Fill 档位组件只能独占一行，防止一行无限堆组件导致整行被过度缩小

### 修复
- 修复编辑器组件无法拖动的问题：组件按下后立即捕获指针到页面，避免被预览区 ScrollViewer 抢走导致拖拽失灵
- 修复窗口最大化时模拟主页被放得过大的问题：预览 Viewbox 限制最大设计尺寸，窗口再大也只等比缩放到上限
- 组件右上角新增删除按钮，删除单个组件不再需要整行删除
- 修复编辑器组件外壳（Border）无背景导致不参与命中测试的问题：为其补上透明背景，使点击选中、拖拽与右上角删除按钮恢复可用
- 修复组件库条目无法拖动的问题：条目控件由 Button 改为 Border，避免 Button 类处理程序吞掉 PointerPressed 导致自定义拖拽处理器收不到事件
- 修复鼠标悬停在组件上的光标在箭头与手型间快速闪烁的问题：在组件上增加透明覆盖层统一命中测试与光标，抑制内部卡片光标/交互泄漏
- 修复整行（Fill）组件时删除整行按钮与删除组件按钮在右上角重合的问题：把行删除按钮移到行左上角
- 修复组件库条目拖入预览失败的问题：落点行的坐标换算补齐 Viewbox 等比缩放，避免矩形判定偏离导致拖放落空
- 删除整行按钮改到行首：行模板改为「按钮列 + 组件列」的 Grid 布局，按钮垂直居中固定于行首而非浮出左上角
- 优化编辑器组件边框：悬停改为描边高亮（不再整块填充背景），选中态增加半透明底色，选中更清晰且拖拽经过时不再把周边组件点亮
- 拖拽组件过程中不再点亮其他组件：按下时不再立即选中，改为未发生拖动的纯点击释放时才选中

### 调整（主页自定义）
- 插件注册的主页卡片不再自动放入主页布局：去掉 `EnsureComponentInRows` 自动追加逻辑，插件卡片先出现在组件库，由用户手动添加
- 组件库不再受卡片「启用」状态拦截：`CreateDataComponentVM` 取消 `IsEnabled` 过滤，禁用的内置卡片（如多人联机）可从组件库正常拖入主页（此前因静默返回 null 而拖不进去）
- 卡片尺寸设为「整行」（Fill）时，若所在行还有其他组件，自动把该组件拆到新独占行再改为 Fill，确保真正铺满整行（此前会被行容量校验静默拒绝）
- 移除预览外部的「＋ 添加一行」按钮：改为最后一行之后的内嵌 add-row 槽位，垂直居中显示一个加号，预览滚动到底部时槽位隐藏；空行保留占位空间
- 拖拽自动滚动：拖动组件或组件库条目时，指针进入预览可视区上/下 36px 边缘带后，由 DispatcherTimer（16ms/帧）按 18px/帧持续滚动预览，直到指针离开边缘带或到达滚动边界
- 修复拖拽自动滚动"完全滑不动"：模拟主页固定 600 高、行数少时预览内部内容不足无法滚动，之前自动滚动因此完全失效；现改为双滚动策略——预览内部可滚先滚预览，预览到底/顶或内容不足时兜底滚动整个设置页，并统一改用 Offset 赋值

### 优化（主页自定义编辑器）
- 重写编辑器拖拽：改为指针捕获 + 跟随光标的提示浮层 + 落点行高亮，拖拽移动/添加稳定可靠
- 行渲染统一为"一排到底"：主页与模拟主页一致，行内放不下时整行等比缩小，不再折行
- 模拟主页改为固定设计尺寸（与真实主页内容区同比例），高度观感与主页一致；新建行自动滚动到可见
- 空行显示占位提示，可正常悬停删除；组件库允许重复添加同一组件，移除"已添加"禁用态

### 新增（主页自定义，第三阶段）
- 设置页新增「主页自定义」页：1:1 模拟主页预览（等比缩放），点击选中组件、组件拖拽跨行移动与行内排序、组件库点击/拖拽添加（含插件注册的组件）、选中组件尺寸调整与删除、行固定到底部/取消固定、增删行、恢复默认布局，所有改动实时持久化并同步到主页

### 修复
- 修复早期迁移写盘的布局缺少底部固定标记的问题：读取布局时自动为分隔线行和操作行补上固定标记，操作区恢复旧版固定在主页底部的效果

### 清理
- 移除通用设置页旧的"主页卡片"管理区块（启用/禁用开关与上下移动排序，已由设置页主页自定义取代）；插件卡片注册/注销时改为刷新主页编辑器的组件库

### 新增（主页自定义基建，第二阶段）
- 主页改为组件化渲染：按持久化布局（行 + 组件 + 尺寸档位）数据驱动渲染，欢迎卡、导航卡片、账号/版本选择、启动按钮、日志开关、分隔线全部为独立组件模板
- 组件运行时视图模型（HomeRowViewModel/HomeComponentViewModel 系列）：行内组件横向排列自动换行，Fill 尺寸组件独占整行；插件卡片注册后自动进入主页布局（与旧版行为一致），插件禁用/卸载自动移除

### 新增（主页自定义基建，第一阶段）
- 新增主页组件注册表 `HomeComponentRegistry`：内置组件（欢迎卡、新闻、多人联机、资源下载、账号选择、版本选择、启动按钮、日志开关、分隔线）与插件卡片、插件自定义组件统一注册，按内置/插件分组供组件库展示
- 新增主页布局模型 `HomeLayout`（行 + 组件 + 尺寸档位 Small/Medium/Large/Fill），旧版 `HomeCards` 配置（启用状态/排序）首次使用时自动迁移为默认布局
- 新增插件 API：`RegisterHomeCard` 支持声明默认尺寸档位（重载）；`RegisterHomeComponent` / `UnregisterHomeComponent` 允许插件注册完全自定义 UI 的主页组件（内容工厂模式）
